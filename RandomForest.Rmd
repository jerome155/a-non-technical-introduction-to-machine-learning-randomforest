...
Random Forests build up on multiple decision trees. The core functionality is therefore the same as for the ID3_Decision Tree algorithm.
Steps 1-3 are Random Forest specific. 4-6 are the same as if used with ID3 Decision Trees.

  1. Start with a single node containing all observations
  2. Draw samples (with replacement) from the original data
  3. For each sample, randomly sample m of the features
  4. Identify best split across m features
  5. Split node into child nodes
  6. Determine per child node if stopping criteria is reached or if a further split is necessary.

This algorithm reuses the implementation of the ID3 Decision Tree. Therefore, these sections are not commented (steps 4-6).
...
Main function of the Random Forest algorithm. Uses the existing ID3 algorithm as basis.
...
Main function to predict new values using multiple decision trees.
...
Sums up the predictions and returns the result.
...
Main code that trains the Random Forest
